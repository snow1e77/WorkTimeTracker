...
```bash
chmod +x *.sh scripts/*.sh check-services-mac.sh
...
**Альтернативные команды npm для macOS:**
```bash
npm run setup:mac          # Полный запуск проекта
npm run stop:mac           # Остановка проекта  
npm run check:services:mac # Проверка сервисов
npm run create:test-worker:mac # Создание тестового пользователя
```
...
Для тестирования функционала мобильного приложения создан специальный тестовый аккаунт:
...
# или
npm run create:test-worker:mac
```
