...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "✅ Тестовый аккаунт успешно создан!" -ForegroundColor Green
        Write-Host ""
        ...
        Write-Host "   4. Создайте профиль с именем" -ForegroundColor Gray
        Write-Host "   5. Готово! Можете тестировать функционал рабочего" -ForegroundColor Gray
        Write-Host ""
        Write-Host "💡 Совет: В режиме разработки всегда используется код 123456" -ForegroundColor Blue
    } else {
