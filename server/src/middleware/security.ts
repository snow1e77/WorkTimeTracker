...
const csrfTokens = new Map<string, { token: string; expires: Date; userId?: string }>();
...
export const csrfProtection = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (['POST', 'PUT', 'DELETE', 'PATCH'].includes(req.method)) {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      if (!origin && !referer) {
        logSecurityEvent('missing_origin_referer', {
          ip: req.ip,
          url: req.originalUrl
        }, 'medium');
        res.status(403).json({
...
export const requirePrivacyConsent = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (!consentHeader || consentHeader !== 'granted') {
      logSecurityEvent('privacy_consent_missing', {
        ip: req.ip,
        consentHeader
      }, 'medium');
      res.status(400).json({
    ...
    if (consentTimestamp) {
      ...
      if (consentDate < oneYearAgo) {
        logSecurityEvent('privacy_consent_expired', {
          ip: req.ip,
