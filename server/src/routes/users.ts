...
const _addPreRegisteredUserSchema = Joi.object({
  ...
  try {
    ...
    for (const userId of userIds) {
      try {
        ...
        errors.push(
          `Error updating user ${userId}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
...
router.post(
  ...
  async (req, res) => {
    return res.status(400).json({
      success: false,
      error:
        'Этот endpoint устарел. Используйте /api/users/register-user для прямой регистрации пользователей',
    });
  }
);
...
router.get('/pre-registered', requireRole('admin'), async (req, res) => {
  return res.status(400).json({
    success: false,
    error:
      'Этот endpoint устарел. Используйте /api/users/all для получения всех пользователей',
  });
});
...
router.delete('/pre-registered/:id', requireRole('admin'), async (req, res) => {
  return res.status(400).json({
    success: false,
    error:
      'Этот endpoint устарел. Используйте /api/users/:id для удаления пользователей',
  });
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await UserService.assignForemanToWorker(
        workerId,
        foremanId
      );
